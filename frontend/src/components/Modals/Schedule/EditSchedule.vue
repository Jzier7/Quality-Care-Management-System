<template>
  <q-dialog v-model="modalStore.showEditScheduleModal">
    <q-card flat bordered class="q-pa-md text-white" style="width: 700px; max-width: 80vw;">
      <h3 class="text-primary pb-4">Edit Schedule</h3>
      <q-form @submit.prevent>
        ...
        <div class="row justify-end">
          <q-btn label="Save Changes" color="primary" @click="editSchedule" />
...
export default {
  ...
  methods: {
    closeModal() {
      this.modalStore.setShowEditScheduleModal(false);
    },
    clearForm() {
      this.localForm = {
        title: '',
    ...
    setFormValues() {
      if (this.editData) {
        const { start, end } = this.editData;
        this.localForm = {
          title: this.editData.title,
          patient: this.editData.patient
            ? {
              id: this.editData.patient.id,
              name: `${this.editData.patient.user.first_name} ${this.editData.patient.user.last_name}`
            }
            : '',
          start_date: start ? start.split(' ')[0].replace(/-/g, '/') : '',
          start_time: start ? start.split(' ')[1] : '',
          end_time: end ? end.split(' ')[1] : '',
        };
      }
    },
    async editSchedule() {
      try {
        ...
        const response = await scheduleService.updateSchedule(scheduleData);
        ...
        Notify.create({
          type: 'negative',
          position: 'top',
          message: error.response?.data?.message || 'Error updating schedule.',
        });
      }
    },
    async fetchPatients() {
